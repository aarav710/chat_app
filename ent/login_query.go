...
func (lq *LoginQuery) Order(o ...OrderFunc) *LoginQuery {
	lq.order = append(lq.order, o...)
	return lq
}

// QueryUser chains the current query on the "user" edge.
func (lq *LoginQuery) QueryUser() *UserQuery {
...
func (lq *LoginQuery) IDs(ctx context.Context) ([]int, error) {
	var ids []int
...
func (lq *LoginQuery) WithUser(opts ...func(*UserQuery)) *LoginQuery {
	query := &UserQuery{config: lq.config}
...
func (lq *LoginQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Login, error) {
	...
	if query := lq.withUser; query != nil {
		...
		query.Where(predicate.User(func(s *sql.Selector) {
			s.Where(sql.InValues(login.UserColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.login_user
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "login_user" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "login_user" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.User = n
		}
	}
...
func (lq *LoginQuery) sqlExist(ctx context.Context) (bool, error) {
...
func (lgb *LoginGroupBy) Scan(ctx context.Context, v interface{}) error {
...
func (lgb *LoginGroupBy) sqlScan(ctx context.Context, v interface{}) error {
...
func (lgb *LoginGroupBy) sqlQuery() *sql.Selector {
...
func (ls *LoginSelect) Scan(ctx context.Context, v interface{}) error {
	if err := ls.prepareQuery(ctx); err != nil {
		return err
	}
	ls.sql = ls.LoginQuery.sqlQuery(ctx)
	return ls.sqlScan(ctx, v)
}

func (ls *LoginSelect) sqlScan(ctx context.Context, v interface{}) error {
	rows := &sql.Rows{}
