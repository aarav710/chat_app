...
func (uq *UserQuery) QueryLogin() *LoginQuery {
	query := &LoginQuery{config: uq.config}
...
func (uq *UserQuery) WithLogin(opts ...func(*LoginQuery)) *UserQuery {
	query := &LoginQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withLogin; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*User)
		for i := range nodes {
			if nodes[i].login_user == nil {
				continue
			}
			fk := *nodes[i].login_user
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "login_user" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Login = n
			}
		}
	}
